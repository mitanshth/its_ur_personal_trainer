...
function forgeFitFormation(move) {
  const name = String(move.name || "").toLowerCase();
  const part = move.part || "Core";
  const rules = [
    [/push|bench press|chest press|fly/, ["Set: hands level with chest, shoulders packed.", "Move: lower slowly, press without shrugging.", "Breathe: inhale down, exhale as you press."]],
    [/row|pull|pulldown|pull-up/, ["Set: chest tall, ribs down, grip firm.", "Move: pull elbows back, squeeze shoulder blades.", "Breathe: exhale on the pull, inhale on return."]],
    [/squat|leg press|step-up|lunge|split/, ["Set: full foot planted, core braced.", "Move: knees track toes, hips and chest rise together.", "Breathe: inhale before lowering, exhale up."]],
    [/deadlift|good morning|hinge|glute|hip/, ["Set: soft knees, spine long, hips loaded.", "Move: push hips back, keep weight close.", "Breathe: brace first, exhale through the finish."]],
    [/shoulder|press|raise|upright|rotation/, ["Set: ribs down, neck relaxed, wrists neutral.", "Move: lift smoothly without swinging.", "Breathe: exhale during effort, control the return."]],
    [/curl|triceps|dip|skull|kickback/, ["Set: elbows fixed, shoulders quiet.", "Move: isolate the arm, avoid body swing.", "Breathe: exhale on the hard half of the rep."]],
    [/plank|dead bug|crunch|sit-up|pallof|wood chop|carry/, ["Set: brace belly like a shield.", "Move: keep hips steady and spine controlled.", "Breathe: short calm breaths, no breath-holding."]],
    [/climber|bound|stairs|treadmill|bike|row|sled|thruster/, ["Set: soft knees and athletic posture.", "Move: stay rhythmic before going faster.", "Breathe: steady nasal-mouth rhythm."]],
    [/stretch|mobility|flexor|hamstring|cat|ankle|90\/90/, ["Set: enter the stretch slowly.", "Move: stop before pain, hold the clean range.", "Breathe: long exhales to unlock tension."]]
  ];
  const match = rules.find(([pattern]) => pattern.test(name));
  const fallback = {
    Chest: ["Set: shoulders packed and chest open.", "Move: control both lowering and lifting.", "Breathe: exhale when effort peaks."],
    Back: ["Set: chest proud and spine long.", "Move: pull with elbows, not neck.", "Breathe: exhale as you squeeze."],
    Legs: ["Set: feet stable and core braced.", "Move: knees follow toes.", "Breathe: inhale down, exhale up."],
    Shoulders: ["Set: ribs down and neck calm.", "Move: smooth path, no swinging.", "Breathe: exhale as you lift."],
    Arms: ["Set: elbows steady.", "Move: slow reps, full control.", "Breathe: exhale on effort."],
    Core: ["Set: brace abs.", "Move: keep hips level.", "Breathe: steady and controlled."],
    Conditioning: ["Set: soft landing.", "Move: smooth rhythm.", "Breathe: never hold your breath."],
    Mobility: ["Set: relax into position.", "Move: gentle range only.", "Breathe: long exhales."]
  }[part] || ["Set: stable position.", "Move: control the rep.", "Breathe: steady rhythm."];
  const steps = match ? match[1] : fallback;
  return `
