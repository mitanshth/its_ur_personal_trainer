...
function getMotivation() {
  ...
  if (style === "calm") return streak > 1 ? `${name}, clear today's quest with clean form and calm focus.` : "Open the gate gently. Ten honest minutes starts the rank-up.";
  if (style === "competitive") return streak > 1 ? `${name}, protect the streak. Hunters do not skip daily quests.` : "Accept the quest. One rep is enough to break the inertia.";
  return streak > 1 ? `${name}, your streak is active. Complete one more quest today.` : "Daily quest available. Train today and make discipline visible.";
}
...
function questNotificationMessage() {
  ...
  const messages = [
    `Daily quest: ${workout.title}. Clear it and protect your streak.`,
    `Rank-up condition: train today, log protein, and hit ${targets.protein}g protein.`,
    `Gate opened: ${workout.bodyParts}. Start with the warm-up, then finish the quest.`,
...
async function enableNotifications() {
  ...
  if (permission === "granted") {
    ...
    saveState();
    scheduleNotification();
    new Notification("Daily quest activated", { body: questNotificationMessage() });
...
function scheduleNotification() {
  ...
  notificationTimer = window.setTimeout(() => {
    new Notification("ForgeFit daily quest", { body: `${questNotificationMessage()} Log sleep, water, soreness, and weight.` });
    scheduleNotification();
  }, target - new Date());
}
