  ...
  return Array.from({ length: Number(daysPerWeek) }, (_, index) => {
    ...
    parts.forEach((part, partIndex) => {
      ...
      Array.from({ length: moveCount }, (_, moveIndex) => {
        ...
        moves.push({ name, dose, part: titleCase(part) });
      });
    });
    ...
    return {
      ...
      moves: trimmedMoves,
      finisher: theme.includes("Mobility") ? "5 minutes nasal breathing" : limited ? "8 minute incline walk" : goal === "fat-loss" ? "8 minute calorie finisher" : "6 minute controlled finisher"
    };
  });
}
...
function renderWorkoutBlock(container, workout) {
  container.innerHTML = `
    ...
    <ul class="exercise-list">
      ${workout.moves.map((move) => `<li><strong>${move.name}</strong><br><span>${move.part} - ${move.dose}</span></li>`).join("")}
